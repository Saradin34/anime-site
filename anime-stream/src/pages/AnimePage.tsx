...
export default function AnimePage() {
  ...
  // Раскрытие списка серий на мобиле (на десктопе всегда открыт)
  ...
  // Throttle для записи в историю: не чаще раза в 10 секунд просмотра
  const lastRecordAtRef = useRef<{ ep: number; sec: number } | null>(null)
  ...
  // + сбрасываем throttle истории, чтобы новая серия сразу записалась.
  useEffect(() => {
    lastRecordAtRef.current = null
  ...
  const handleTimeUpdate = (cur: number, dur: number) => {
    ...
    // dur может быть 0 или Infinity пока метаданные HLS не догрузились.
    // Считаем прогресс только если он валиден; иначе просто фиксируем сам факт просмотра.
    const hasValidDur = Number.isFinite(dur) && dur > 0
    const progress = hasValidDur ? Math.min(1, cur / dur) : 0

    // Записываем не чаще раза в 10 секунд просмотра ЭТОЙ серии.
    // Работает даже если пользователь мотает — сравниваем по разнице,
    // а не по конкретной секунде (как было раньше — ловили только 0,15,30…)
    const last = lastRecordAtRef.current
    const shouldRecord =
      !last ||
      last.ep !== currentEp ||
      Math.abs(cur - last.sec) >= 10

    if (!shouldRecord) return
    // Минимальный порог: 5 секунд просмотра, чтобы случайные клики не засоряли историю
    if (cur < 5) return

    lastRecordAtRef.current = { ep: currentEp, sec: cur }
      ...
      <div className="max-w-[1500px] mx-auto px-4 lg:px-8 -mt-48 relative z-10">
        ...
        {episodes.length > 0 ? (
          <div id="player" className="grid lg:grid-cols-[1fr_320px] gap-6 mb-12">
            ...
            <div className="glass rounded-2xl p-3 flex flex-col lg:max-h-[700px]">
              {/* Заголовок: на мобиле — кнопка-toggle, на десктопе — просто заголовок */}
              <button
                type="button"
                onClick={() => setEpisodesOpen((v) => !v)}
                className="font-semibold p-2 mb-2 flex items-center gap-2 lg:pointer-events-none w-full text-left"
              ...
              >
                ...
                <ChevronDown
                  size={16}
                  className={clsx(
                    'ml-auto lg:hidden transition-transform',
                    episodesOpen && 'rotate-180',
                  )}
                />
              </button>
              {/* Список серий: на десктопе всегда виден (flex-1), на мобиле — по клику */}
              <div
                className={clsx(
                  'overflow-y-auto space-y-1 pr-1 flex-1',
                  // На мобиле: скрыт если !episodesOpen, ограничен по высоте если открыт
                  episodesOpen ? 'block max-h-[60vh]' : 'hidden',
                  // На десктопе: всегда виден, без max-h (родитель имеет lg:max-h-[700px])
